...
import { RefreshCw, CloudSun, Thermometer } from "lucide-react";

// ========== TYPE DEFINITIONS ==========

// Represents a single garment item in the outfit
interface Garment {
	id: string;
	name: string;
	brand: string | null;
	category: string;
	image_url: string | null; // Allow for potentially missing images
}

// The structure of the outfit data prop
type OutfitData = {
	base?: Garment;
	mid?: Garment;
	outer?: Garment;
 ...
 */
const GarmentCard: React.FC<{ garment: Garment; className?: string }> = ({ garment, className = "" }) => {
	return (
...
export const OutfitCard: React.FC<OutfitCardProps> = ({ outfitData, weather, reasoning, onRegenerate }) => {
	...
	const validGarments = (Object.keys(outfitData) as Array<keyof OutfitData>).map((key) => outfitData[key]).filter((g): g is Garment => !!g);
	...
	const getGarmentSpan = (key: keyof OutfitData) => {
		if (garmentCount === 5) {
			return ["outer", "mid", "base"].includes(key) ? "col-span-2" : "col-span-3";
		}
		return "";
	};
		...
		<div className="w-full max-w-4xl mx-auto rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-lg p-6">
			...
			{garmentCount > 0 ? (
				<div className={getGridConfig()}>
					{outfitData.outer && (
						<GarmentCard
							garment={outfitData.outer}
							className={getGarmentSpan("outer")}
						/>
					)}
					{outfitData.mid && (
						<GarmentCard
							garment={outfitData.mid}
							className={getGarmentSpan("mid")}
						/>
					)}
					{outfitData.base && (
						<GarmentCard
							garment={outfitData.base}
							className={getGarmentSpan("base")}
						/>
					)}
					{outfitData.bottom && (
						<GarmentCard
							garment={outfitData.bottom}
							className={getGarmentSpan("bottom")}
						/>
					)}
					{outfitData.shoes && (
						<GarmentCard
							garment={outfitData.shoes}
							className={getGarmentSpan("shoes")}
						/>
					)}
				</div>
			) : (
			...
			<div className="mt-8">
				...
				<p className="text-slate-600 dark:text-slate-300/90 leading-relaxed italic font-serif">&ldquo;{reasoning}&rdquo;</p>
			</div>
