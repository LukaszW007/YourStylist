...
export async function updateUserProfile(
	...
	const { data, error } = await supabaseBrowser.from("user_profiles").update(updates).eq("id", userId).select().single();
...
export async function getUserPreferences(userId: string): Promise<UserPreferences | null> {
	const { data, error } = await supabaseBrowser.from("user_preferences").select("*").eq("user_id", userId).single();
 ...
 */
export async function updateUserPreferences(userId: string, updates: Partial<UserPreferences>): Promise<UserPreferences | null> {
	const { data, error } = await supabaseBrowser.from("user_preferences").update(updates).eq("user_id", userId).select().single();
...
export async function getGarments(
	...
	if (filters?.color) {
		query = query.eq("main_color_name", filters.color);
	}
	if (filters?.brand) {
...
export async function updateGarment(
	...
	const { data, error } = await supabaseBrowser.from("garments").update(updates).eq("id", garmentId).select().single();
...
export async function markGarmentAsWorn(garmentId: string): Promise<Garment | null> {
	const { data: garment } = await supabaseBrowser.from("garments").select("*").eq("id", garmentId).single();
