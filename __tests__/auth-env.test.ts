	...
	return {
		createClient: vi.fn(() => ({
			auth: {
				...
				onAuthStateChange: vi.fn((_cb: (event: string, session: { access_token?: string } | null) => void) => ({
...
describe("Auth + Env safety", () => {
	...
	it("isSupabaseConfigured false when vars missing", async () => {
		// use relative path import to ensure resolution even if alias misconfigured
		const { isSupabaseConfigured } = await import("../src/lib/supabase/config-check");
		expect(isSupabaseConfigured()).toBe(false);
	});

	it("clientEnv.isSupabaseConfigured false and auth functions noop without throwing", async () => {
		const { clientEnv } = await import("../src/env");
		const { signInWithGoogle, signOut, getSession, getCurrentUser } = await import("../src/lib/supabase/auth");
	...
	it("auth functions operate when env present", async () => {
		...
		const { clientEnv } = await import("../src/env");
		const { signInWithEmail, signUpWithEmail, signInWithGoogle, signOut, getSession, getCurrentUser } = await import("../src/lib/supabase/auth");
